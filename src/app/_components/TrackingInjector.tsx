  ...
  switch (provider) {
    ...
    case 'TikTokPixel':
      return `
        // TikTok Pixel
        !function (w, d, t) {
          w.TiktokAnalyticsObject=t;var tt=w[t]=w[t]||[];tt.methods=['page','track','identify','instances','debug','on','off','once','ready','alias','group','enableCookie'];tt.setAndDefer=function(t,e){t[e]=function(){t.push([e].concat(Array.prototype.slice.call(arguments,0)))}};for(var i=0;i<tt.methods.length;i++)tt.setAndDefer(tt,tt.methods[i]);tt.instance=function(t){var e=tt._i[t]||[];for(var n=0;n<tt.methods.length;n++)tt.setAndDefer(e,tt.methods[n]);return e};tt.load=function(e){var n='https://analytics.tiktok.com/i18n/pixel/events.js';tt._i=tt._i||{};tt._i[e]=[];var a=d.createElement('script');a.type='text/javascript';a.async=!0;a.src=n+'?sdkid='+e+'&lib='+t;var s=d.getElementsByTagName('script')[0];s.parentNode.insertBefore(a,s)};tt.load('${id}');tt.page();
        }(window, document, 'ttq');
      `.trim();
...
/** วางไว้ใน layout หรือในหน้า Store (ส่ง storeId) */
export default function TrackingInjector({ storeId }: { storeId?: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const url = storeId ? `/api/tracking-scripts?storeId=${encodeURIComponent(storeId)}` : '/api/tracking-scripts';
  ...
  if (!loaded || !rendered) return null;
  return <>{rendered}</>;
}